...
function [par]=parameters(Areas,fln,sln,wiring,hierVals,G,Wplus,cutFB)
...
par.dt=0.5e-3;par.triallength=25.;par.transient=1.;
...
Jnsmin=0.21;Jnsmax=Wplus; %all areas monostable [0.21 0.42]
...
if cutFB==1 %we cut the FB projections (in the sense of spine count gradient) if necessary
    [~,newindex]=sort(Jnsgrad);%flnM=flnM(newindex);flnM=tril(flnM);
    for i=1:Nareas % new post
        for j=1:Nareas % new pre
            oldranki=newindex(i);
            oldrankj=newindex(j);
            if j>i
                flnM(oldranki,oldrankj)=0;
            end
        end
    end
end
...
for i=1:2 %we consider as targets 8m and 8l
    for j=1:length(frontal)
        x=frontal(i);
        y=frontal(j);
        maxi=0.4; %this means long-range inhibition to FEF tops at maxi=0.25
        %lambdai(x,y)
        if lambdai(x,y)>maxi
            lambdai(x,y)=maxi;
        end
